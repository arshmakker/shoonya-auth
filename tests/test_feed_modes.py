...
def test_cache_serving_when_hybrid_or_ws_then_true():
    assert cache_serving_for("hybrid") is True
    assert cache_serving_for("ws") is True
...
def test_validator_when_ws_mode_then_off_but_cache_still_serves():
    """'ws' is hybrid minus the referee: serve from cache, make no periodic
    REST calls. Only for days where WS accuracy is not being adjudicated."""
    assert cache_serving_for("ws") is True
    assert validator_runs_for("ws") is False
